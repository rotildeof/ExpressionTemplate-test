...
struct Plus {
    static double apply(const double l, const double r) { return l + r; }
};
...
class Vect {
  ...
  public:
    ...
    // }
    template <class R> Vect &operator=(const R &rhs) {
...
template <class L, class R> Exp<L &&, Plus, R &&> operator+(L &&l, R &&r) {
